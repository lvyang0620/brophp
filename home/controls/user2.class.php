<?php
	class User{
		function index(){
			...
			$cat=D("cat");
			//$data=$cat->field('id,name as catname,desn')->r_select(array('article','id as a_id,cid,name as a_name','cid',array("arts",'id desc',5)),array("test",'',cid,array("tst",null,2)));
			$data=$cat->where("3")->r_delete(array("article","cid"),array("test","cid"));
			p($data);
		}
